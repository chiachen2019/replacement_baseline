...
simulation_results <- simulate_multiple_wolbachia_matrix(param_matrix)

## ---- 3. Add fold-change in VC (perc_diff_vc) ----

# Use the release_start from the *first* row (same as param_matrix[1,19] before)
...
write.csv(
  combined_df,
  paste0("temporal_output_all", Sys.Date(), ".csv"),
  row.names = FALSE
)
