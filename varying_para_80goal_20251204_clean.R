...
simulate_80percent_with_varying_params <- function(param_matrix, param_names, values_list) {
  ...
  results <- simulate_multiple_wolbachia_80percent_matrix(varied_param_matrix)
...
param_matrix <- matrix(c(
  ...
  0.2147, 1, 0.9702003, 9.50689, 21 #this number of days wont matter. 
), ncol = 19, byrow = TRUE)
...
values_list <- list(seq(46183.47*0.05,(46183.47), by = 46183.47 * 0.025), 
                    seq(0, 0.5, by = 0.1))  
# Run the simulation
sim_results <- simulate_80percent_with_varying_params(param_matrix, param_names, values_list)
...
write.csv(final_results, file = paste0("final_results_goal80", Sys.Date(),".csv"))
